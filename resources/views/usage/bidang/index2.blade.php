            ...
            <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                ...
                <div class="flex items-center space-x-2">
                    ...
                    <input type="text" id="search"
                        class="border border-gray-300 rounded-md px-2 py-1 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500">
                    <a href="{{ routeForRole('asset-usage', 'create') }}"
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        @foreach ($usages as $index => $usage)
                            <tr>
                                ...
                                <td class="text-center px-4 py-3 text-sm text-gray-900 whitespace-normal break-words">{{ $usage->nama }}</td>
                                <td class="text-center px-4 py-3 text-sm text-gray-900 whitespace-normal break-words">{{ $usage->kepala->nama ?? '-' }}</td>
                                <td class="text-center px-4 py-3 text-sm text-gray-900 whitespace-normal break-words">{{ $usage->lokasi ?? '-' }}</td>
                                <td class="text-center px-4 py-3 text-sm text-gray-900 whitespace-normal break-words">{{ $usage->institution->nama ?? '-' }}
                                </td>
                                <td class="text-center px-4 py-3 text-sm">
                                    <div class="flex items-center justify-center gap-x-3">
                                        ...
                                        <form method="POST" action="{{ routeForRole('asset-usage', 'destroy', $usage->id) }}" class="delete-form">
                                            @csrf
                                            @method('DELETE')
