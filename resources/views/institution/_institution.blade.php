...
<div>
    ...
    <input type="text" id="alias" name="alias" value="{{ old('alias', $institution->alias ?? '') }}"
        class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 text-gray-900 dark:text-white rounded-md focus:outline-none focus:ring-2 focus:ring-indigo-500" {{ isset($institution) && $institution->exists ? 'bg-gray-100' : '' }}"
        {{ isset($institution) && $institution->exists ? 'disabled' : '' }} required>
</div>
<div>
    <label for="pemerintah" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Pemerintah <span class="text-red-500">*</span></label>
...
<div class="form-group">
    ...
    <input type="email" name="email" id="email" value="{{ old('email', $institution->email ?? '') }}"  placeholder="Email" required
        autofocus class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 text-gray-900 dark:text-white rounded-md focus:outline-none focus:ring-2 focus:ring-indigo-500" @error('email') is-invalid @enderror">
    @error('email')
...
<div>
    ...
    @if($employees->isNotEmpty())
        ...
        <select 
            name="kepala_instansi_id" 
            id="kepala_instansi_id"
    ...
    @else
        ...
        <div class="mt-1 block w-full py-2 px-3 
