...
<div class="space-y-8">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 px-4">
        @php
            $summary = [
                ...
                ['title' => 'Total Pegawai', 'value' => $stats['pegawai'], 'icon' => 'M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M13 7a4 4 0 11-8 0 4 4 0 018 0z', 'color' => 'from-emerald-500 to-teal-500', 'change' => '+5', 'changeType' => 'up'],
                ['title' => 'Akun Aktif', 'value' => $stats['akun_aktif'], 'icon' => 'M10.325 4.317c.426-1.756 2.924-1.756 3.35 0a1.724 1.724 0 002.573 1.066c1.543-.94 3.31.826 2.37 2.37a1.724 1.724 0 001.065 2.572c1.756.426 1.756 2.924 0 3.35a1.724 1.724 0 00-1.066 2.573c.94 1.543-.826 3.31-2.37 2.37a1.724 1.724 0 00-2.572 1.065c-.426 1.756-2.924 1.756-3.35 0a1.724 1.724 0 00-2.573-1.066c-1.543.94-3.31-.826-2.37-2.37a1.724 1.724 0 00-1.065-2.572c-1.756-.426-1.756-2.924 0-3.35a1.724 1.724 0 001.066-2.573c-.94-1.543.826-3.31 2.37-2.37.996.608 2.296.07 2.572-1.065z', 'color' => 'from-violet-500 to-purple-500', 'change' => '-1', 'changeType' => 'down'],
                ['title' => 'Total Aset', 'value' => $stats['aset'], 'icon' => 'M19 11H5m14 0a2 2 0 012 2v6a2 2 0 01-2 2H5a2 2 0 01-2-2v-6a2 2 0 012-2m14 0V9a2 2 0 00-2-2M9 11a2 2 0 114 0m-4 0V9a2 2 0 012-2m0 0V5a2 2 0 012-2m-2 0V1', 'color' => 'from-orange-500 to-amber-500', 'change' => '+12', 'changeType' => 'up'],
            ];
        @endphp

        @foreach($summary as $card)
            <div class="bg-white dark:bg-gray-800 shadow-lg hover:shadow-xl transition-all duration-300 hover:-translate-y-1 rounded-xl overflow-hidden">
                <div class="p-6">
                    <div class="flex items-start justify-between">
                        ...
                        <div class="w-14 h-14 bg-gradient-to-br {{ $card['color'] }} rounded-2xl flex items-center justify-center shadow-lg">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 px-4">
        ...
        <div class="lg:col-span-2 bg-white dark:bg-gray-800 shadow-lg rounded-xl p-6">
            ...
            <div class="space-y-4">
                @foreach($asetPerBidang as $index => $item)
                    @php
        ...
        <div class="bg-white dark:bg-gray-800 shadow-lg rounded-xl p-6">
            ...
            <div class="space-y-4">
                ...
                <div class="flex items-center justify-between p-4 bg-gradient-to-r from-emerald-50 to-teal-50 dark:from-emerald-900/20 dark:to-teal-900/20 rounded-xl">
                    <div>
                        <p class="text-sm text-gray-600 dark:text-gray-400">Aset Tidak Bergerak</p>
                        <p class="text-2xl font-bold text-gray-900 dark:text-white">{{ $stats['aset_tidak_bergerak'] }}</p>
                    </div>
                ...
                <div class="flex items-center justify-between p-4 bg-gradient-to-r from-violet-50 to-purple-50 dark:from-violet-900/20 dark:to-purple-900/20 rounded-xl">
                    <div>
                        <p class="text-sm text-gray-600 dark:text-gray-400">Aset Habis Pakai</p>
                        <p class="text-2xl font-bold text-gray-900 dark:text-white">{{ $stats['aset_habis_pakai'] }}</p>
                    </div>
                    <svg class="w-8 h-8 text-violet-500" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
        ...
        <div class="bg-white dark:bg-gray-800 shadow-lg rounded-xl p-6">
            ...
            <div class="space-y-3">
                @foreach($upcomingSchedules as $schedule)
                    <div class="flex items-center justify-between p-4 rounded-xl bg-gray-50 dark:bg-gray-700/50 hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors">
                        <div class="flex items-center space-x-3">
                            ...
                            <div>
                                <p class="font-medium text-gray-900 dark:text-white">{{ $schedule->departement->nama ?? '—' }}</p>
                                <p class="text-sm text-gray-500 dark:text-gray-400">{{ \Carbon\Carbon::parse($schedule->tanggal)->translatedFormat('d F Y') }}</p>
                            </div>
                        </div>
                    ...
                    <div class="flex items-start space-x-3">
                        ...
                        <div class="flex-1">
                            <p class="text-sm font-medium text-gray-900 dark:text-white">
                                {{ ucfirst($activity->event ?? 'Aktivitas') }} {{ class_basename($activity->subject_type ?? '') }}
                            </p>
    ...
    <div class="bg-white dark:bg-gray-800 shadow-lg rounded-xl p-6 mx-4">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div class="text-center p-6 bg-gradient-to-br from-emerald-50 to-teal-50 dark:from-emerald-900/20 dark:to-teal-900/20 rounded-2xl">
                <div class="text-4xl font-bold text-emerald-600 dark:text-emerald-400 mb-2">{{ $stockStatus['selesai'] }}</div>
                <p class="text-sm font-medium text-gray-600 dark:text-gray-400">Selesai</p>
                <div class="mt-2 text-2xl">{{ $stockStatus['total'] ? round(($stockStatus['selesai'] / $stockStatus['total']) * 100) : 0 }}%</div>
            </div>
            <div class="text-center p-6 bg-gradient-to-br from-amber-50 to-orange-50 dark:from-amber-900/20 dark:to-orange-900/20 rounded-2xl">
                <div class="text-4xl font-bold text-amber-600 dark:text-amber-400 mb-2">{{ $stockStatus['berlangsung'] }}</div>
                <p class="text-sm font-medium text-gray-600 dark:text-gray-400">Sedang Berlangsung</p>
                <div class="mt-2 text-2xl">{{ $stockStatus['total'] ? round(($stockStatus['berlangsung'] / $stockStatus['total']) * 100) : 0 }}%</div>
            </div>
            <div class="text-center p-6 bg-gradient-to-br from-gray-50 to-gray-100 dark:from-gray-800/30 dark:to-gray-700/30 rounded-2xl">
                <div class="text-4xl font-bold text-gray-600 dark:text-gray-400 mb-2">{{ $stockStatus['belum'] }}</div>
                <p class="text-sm font-medium text-gray-600 dark:text-gray-400">Belum Dimulai</p>
                <div class="mt-2 text-2xl">{{ $stockStatus['total'] ? round(($stockStatus['belum'] / $stockStatus['total']) * 100) : 0 }}%</div>
            </div>
        </div>
    </div>
    ...
    <div class="p-4 bg-white dark:bg-gray-800 shadow rounded-lg overflow-hidden mx-4">
        ...
        <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-4 gap-4">
            ...
            <form action="{{ routeForRole('activity', 'export') }}" method="post" class="inline-flex items-center justify-center w-1/2 sm:w-auto">
                @csrf
                <button type="submit" class="inline-flex items-center justify-center w-full px-3 py-2 text-sm font-medium text-center text-white bg-blue-600 border border-blue-600 rounded-lg hover:bg-blue-700 focus:ring-4 focus:ring-blue-300 dark:bg-blue-700 dark:hover:bg-blue-600 dark:focus:ring-blue-800">
                    <svg class="w-5 h-5 mr-2 -ml-1" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
                        <path fill-rule="evenodd" d="M3 3a2 2 0 012-2h10a2 2 0 012 2v7a1 1 0 11-2 0V3H5v14h5a1 1 0 110 2H5a2 2 0 01-2-2V3zm9 10a1 1 0 10-2 0v3.586l-1.293-1.293a1 1 0 00-1.414 1.414l3 3a1 1 0 001.414 0l3-3a1 1 0 00-1.414-1.414L12 16.586V13z" clip-rule="evenodd"></path>
                    </svg>
                    Export LOG
                </button>
            </form>
            <div class="flex items-center space-x-2">
                ...
                <input type="text" id="activity-search" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-500 focus:border-primary-500 block p-2 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white" placeholder="Cari aktivitas...">
            </div>
        </div>
            ...
            <table class="min-w-full divide-y divide-gray-200 table-fixed dark:divide-gray-600">
                ...
                <thead class="bg-gray-100 dark:bg-gray-700">
                    <tr>
                        ...
                        <th class="px-4 py-3 text-center text-xs font-medium text-gray-900 uppercase tracking-wider dark:text-white">Perubahan</th>
                    </tr>
                </thead>
                <tbody class="bg-white divide-y divide-gray-200 dark:bg-gray-800 dark:divide-gray-700" id="activity-body">
                    @forelse ($activities as $index => $activity)
                        <tr class="hover:bg-gray-100 dark:hover:bg-gray-700 align-top">
                            ...
                            <td class="p-4 text-sm text-gray-900 dark:text-gray-300 text-center">{{ $activity->created_at->format('Y-m-d H:i:s') }}</td>
                            <td class="p-4 text-sm text-gray-900 dark:text-gray-300 text-center">{{ $activity->causer?->email ?? 'System' }}</td>
                            <td class="p-4 text-sm text-gray-900 dark:text-gray-300 text-center">{{ class_basename($activity->subject_type) }}</td>
                            <td class="p-4 text-sm text-gray-900 dark:text-gray-300 text-center">{{ $activity->subject_id }}</td>
                            <td class="p-4 text-sm text-center">
                                @php
                                    $colors = [
                                        ...
                                        'Stock Opname' => 'bg-blue-100 text-blue-800 dark:bg-blue-700 dark:text-blue-100',
                                    ];
                                @endphp
                            ...
                            <td class="p-4 text-sm text-gray-900 dark:text-gray-300" x-data="{ open: false }">
                                @if (isset($activity->properties['attributes']))
                                    <button @click="open = !open" class="text-xs font-medium text-indigo-600 hover:text-indigo-800 dark:text-indigo-400 dark:hover:text-indigo-300 focus:outline-none mb-1">
                                        <span x-show="!open">Lihat Detail</span>
                                        <span x-show="open">Sembunyikan</span>
                                    </button>
                                    <ul x-show="open" x-transition class="list-disc list-inside space-y-1 mt-1 border-t border-gray-200 dark:border-gray-700 pt-2">
                                        @foreach ($activity->properties['attributes'] as $field => $newValue)
                                            @php $oldValue = $activity->properties['old'][$field] ?? '—'; @endphp
                                            <li>
                                                <span class="font-medium text-indigo-700 dark:text-indigo-400">{{ ucfirst($field) }}:</span>
                                                <span class="text-gray-700 dark:text-gray-300">"{{ $oldValue }}" → "{{ $newValue }}"</span>
                                            </li>
                                        @endforeach
                                    </ul>
                                @else
                    ...
                    @empty
                        <tr>
                            <td colspan="8" class="p-4 text-center text-gray-500 dark:text-gray-400">Belum ada aktivitas tercatat.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
