...
@section('content')
    <div class="space-y-8">
                    ...
                    @foreach ($asetPerBidang as $index => $item)
                        ...
                        <div class="space-y-2">
                            <div class="flex justify-between text-sm">
                                ...
                                <span class="text-gray-500 dark:text-gray-400">{{ $item->jumlah }} aset
        ...
        <div class="p-6 bg-white dark:bg-gray-800 shadow rounded-lg overflow-hidden mx-4 mb-8">
            <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-6 gap-4">
                <h2 class="text-lg font-semibold text-gray-900 dark:text-white">
                    Riwayat Aktivitas Sistem (Lengkap)
                </h2>

                <div class="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 w-full sm:w-auto">
                    <form action="{{ routeForRole('activity', 'export') }}" method="post" class="flex-shrink-0">
                        @csrf
                        <button type="submit"
                            class="inline-flex items-center justify-center w-full sm:w-auto px-3 py-2 text-sm font-medium text-white bg-blue-600 border border-blue-600 rounded-lg hover:bg-blue-700 focus:ring-4 focus:ring-blue-300 dark:bg-blue-700 dark:hover:bg-blue-600 dark:focus:ring-blue-800">
                    ...
                    <div class="flex items-center space-x-2 w-full sm:w-auto">
                        ...
                        <input type="text" id="activity-search"
                            class="flex-1 bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-500 focus:border-primary-500 p-2 dark:bg-gray-700 dark:border-gray-600 dark:text-white"
            ...
            <div class="overflow-x-auto">
                <table class="min-w-full divide-y divide-gray-200 table-fixed dark:divide-gray-600">
                    <thead class="bg-gray-100 dark:bg-gray-700">
                        <tr>
                            <th
                                class="px-4 py-3 text-center text-xs font-semibold text-gray-900 uppercase tracking-wider dark:text-white">
                                No</th>
                            <th
                                class="px-4 py-3 text-center text-xs font-semibold text-gray-900 uppercase tracking-wider dark:text-white">
                                Tanggal</th>
        ...
        <div class="p-6 bg-white dark:bg-gray-800 shadow rounded-lg overflow-hidden mx-4 mb-8">
            <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-6 gap-4">
                <h2 class="text-lg font-semibold text-gray-900 dark:text-white">Riwayat Peminjaman Aset</h2>
                <form action="{{ routeForRole('borrowinglog', 'export') }}" method="post" class="flex-shrink-0">
                    @csrf
                    <button type="submit"
                        class="inline-flex items-center justify-center w-full sm:w-auto px-3 py-2 text-sm font-medium text-white bg-blue-600 border border-blue-600 rounded-lg hover:bg-blue-700 focus:ring-4 focus:ring-blue-300 dark:bg-blue-700 dark:hover:bg-blue-600 dark:focus:ring-blue-800">
                        <svg class="w-5 h-5 mr-2" fill="currentColor" viewBox="0 0 20 20"
                        ...
                        Export LOG Peminjaman
                    </button>
                </form>
                <div class="flex items-center space-x-2 w-full sm:w-auto">
                    <label for="borrowing-search"
                        class="text-sm font-medium text-gray-700 dark:text-gray-300">Cari:</label>
                    <input type="text" id="borrowing-search"
                        class="flex-1 bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-500 focus:border-primary-500 p-2 dark:bg-gray-700 dark:border-gray-600 dark:text-white"
            ...
            <div class="overflow-x-auto">
                <table class="min-w-full divide-y divide-gray-200 table-fixed dark:divide-gray-600">
                    ...
                    <tbody class="bg-white divide-y divide-gray-200 dark:bg-gray-800 dark:divide-gray-700">
                        @forelse ($borrowingLogs as $index => $activity)
                            @php
                                $asset = $activity->subject?->asset;
                                $user = $activity->subject?->user;
                                $dept = $activity->subject?->department;
                            @endphp
                            <tr class="hover:bg-gray-100 dark:hover:bg-gray-700 align-top">
                                <td class="p-4 text-sm text-center text-gray-900 dark:text-gray-300">
                                    {{ $index + $borrowingLogs->firstItem() }}</td>
                                <td class="p-4 text-sm text-center text-gray-900 dark:text-gray-300">
                                    {{ $activity->created_at->format('Y-m-d H:i:s') }}</td>
                                <td class="p-4 text-sm text-center text-gray-900 dark:text-gray-300">
                                    {{ $user->nama ?? 'Unknown User' }}</td>
                                <td class="p-4 text-sm text-center text-gray-900 dark:text-gray-300">
                                    {{ $asset->nama_aset ?? 'Unknown Asset' }}</td>
                                <td class="p-4 text-sm text-center text-gray-900 dark:text-gray-300">
                                    {{ $asset->kode ?? '-' }}</td>
                                <td class="p-4 text-sm text-center text-gray-900 dark:text-gray-300">
                                    {{ $dept->nama ?? 'N/A' }}</td>
                        ...
                        @empty
                            <tr>
                                <td colspan="8" class="p-4 text-center text-gray-500 dark:text-gray-400">
                                    Belum ada aktivitas peminjaman aset.
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
            ...
            @if ($borrowingLogs->hasPages())
                <div class="mt-6 flex flex-col sm:flex-row sm:items-center sm:justify-between">
                    <div class="text-sm text-gray-700 dark:text-gray-300 mb-3 sm:mb-0">
                        Menampilkan <span class="font-medium">{{ $borrowingLogs->firstItem() }}</span> sampai
                        <span class="font-medium">{{ $borrowingLogs->lastItem() }}</span> dari
                        <span class="font-medium">{{ $borrowingLogs->total() }}</span> hasil
                    </div>
